...
We train a JumpReLU sparse autoencoder (32 → 1,024) on the graph embeddings of a pretrained molecular GNN ([GEM](https://www.nature.com/articles/s42256-021-00438-4)) using 2.2M ZINC15 molecules. The sparse features retain the information in the dense embedding and, unlike individual GEM neurons, align with drug classes.

## Results

**Information is preserved.** The SAE reaches an explained variance of 0.9999 with about 52 of 1,024 features active per molecule. An MLP head trained on frozen features gives nearly identical ROC-AUC for the original embedding, the sparse code, and the reconstruction (scaffold split, mean over 5 seeds):
...
**Individual SAE features separate drug classes far better than individual GEM neurons.** Cohen's *d* between target-class and other molecules, for the SAE feature and for the best (sign-swept) of the 32 GEM neurons:
...
**Each feature maps to a chemical scaffold.** The molecules that activate f690, f819 and f675 most strongly are prednisolone esters, fluorinated ethers, and anthraquinone amino-sugars:
